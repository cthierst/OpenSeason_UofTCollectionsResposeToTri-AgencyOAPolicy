...
openAlex_data <- oa_fetch(
  ...
  authorships.institutions.id = "https://openalex.org/I185261750", # UofT ID
)
```
...
openAlex_data_utca <- oa_fetch(
  ...
  from_publication_date = "2019-01-01",
  to_publication_date = "2024-12-31",
  corresponding_institution_ids = "https://openalex.org/I185261750" # UofT ID
)
```
...
publisher_list <- list("John Wiley and Sons Inc","John Wiley and Sons Ltd") # UPDATE Change this text to a list of the publisher's journals
...
wiley_merged2 <- publisher_merged_sorted %>%
  group_by(DOI) %>%
  summarize(
...
wiley_merged2 <- wiley_merged2[, !colnames(wiley_merged2) %in% "Title_lower"]

# Limit all character columns to 150 characters
wiley_merged2[] <- lapply(wiley_merged2, function(col) {
  if (is.character(col)) {
    substr(col, 1, 300)
  } else {
    col
...
wiley_merged2 <-
  wiley_merged2[!grepl("book|editorial|Early",
                    wiley_merged2$`Document Type`,
                    ignore.case = TRUE), ]

# Shorten author lists if they are > 10 names
wiley_merged2$Authors <- sapply(strsplit(wiley_merged2$Authors, ";"), function(authors) {
...
source_counts <- table(wiley_merged2$Source)
...
wiley_merged <- wiley_merged2 %>% # UPDATE "publisher_merged" to match current publisher i.e., csp_merged
...
wiley_merged <- wiley_merged %>%  # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here
...
```{r}
wiley_merged <- wiley_merged[, col_order_final] # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here

write_csv(wiley_merged, "./Output/wiley_merged.csv") # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here
write_xlsx(wiley_merged, "./Output/wiley_merged.xlsx")  # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here
```

```{r}
# Make a new CSV where UofT is the corresponding institution (only Scopus and WOS)
wiley_merged_utca <-  # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here
  wiley_merged [grepl("Univ Toronto|utoronto|University of Toronto", wiley_merged$`Corresponding Author`, ignore.case =  TRUE), ] # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here

# Count rows for each unique Source
source_counts <- table(wiley_merged_utca$Source) # UPDATE "publisher_merged" to match current publisher i.e., csp_merged

# Display the counts
source_counts

write_csv(wiley_merged_utca, "./Output/wiley_merged_utca.csv") # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here
write_xlsx(wiley_merged_utca,"./Output/wiley_merged_utca.xlsx") # UPDATE "publisher_merged" to match current publisher i.e., csp_merged *Make sure you update both here
```
